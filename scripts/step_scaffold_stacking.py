"""Strategy 1 (cross-model stacking) on scaffold-split, all 13 endpoints × N=10.

Replaces strategy1_final_enhanced.pkl semantics for scaffold-primary analysis.
Output: results/scaffold_strategy1.pkl

Source models: 12 non-target endpoints, FULL source data (not scaffold-split,
matching deployment practice — sources are inputs, target is the evaluation
endpoint).  Fixed hyperparameters identical to §2.4.

Target stacking: TPE 15 evals + 3-fold CV on the 2,060-d augmented features.
Ensemble: OOF λ-grid {0.2..0.8} via 3-fold CV (same as random-split S1).
"""
...
def main():
    print("=" * 70, flush=True)
    print(f"SCAFFOLD STACKING (S1)  13 endpoints × N={N_REPS}", flush=True)
    print("=" * 70, flush=True)
    t0 = time.time()
    ...
    out_path = os.path.join(RESULTS_DIR, 'scaffold_strategy1.pkl')
    results = load_dict(out_path)

    # Source models: trained on each source's per-rep SCAFFOLD-SPLIT TRAIN data
    # (mirrors random-split protocol where sources use splits_record[ep][rep][0]).
    # This keeps the only protocol difference vs random-split S1 the SPLIT METHOD.
    ...
    for rep in range(N_REPS):
        seed = 101 + rep
        t_rep = time.time()
        # Check if this rep is complete across all endpoints
        ...
        for ep_target in ENDPOINTS:
            ...
            t_ep = time.time()
            X = datasets[ep_target]['X']
            y = datasets[ep_target]['y']
            ...
            # Final models: yp_base loaded from full TPE50+5fold baseline pkl;
            # yp_stack still computed here (TPE15+3fold on augmented features).
        ...
        # Clear source cache at end of each rep so memory doesn't accumulate
        src_cache.clear()
